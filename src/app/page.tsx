  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', bottom: '10%', left: '10%', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(139,92,246,0.1) 0%, transparent 70%)', pointerEvents: 'none' }} />

        <div className="container" style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 16px', background: 'rgba(99,102,241,0.1)', border: '1px solid var(--border-medium)', borderRadius: '100px', fontSize: '0.8rem', color: 'var(--accent-primary)', marginBottom: '32px', fontWeight: 600 }}>
            ✨ i+1 원리 기반 한국어 읽기
          </div>

          <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)', fontWeight: 900, lineHeight: 1.1, marginBottom: '24px', letterSpacing: '-0.02em' }}>
            한국어를{' '}
          ...
          <p style={{ fontSize: '1.2rem', color: 'var(--text-secondary)', maxWidth: '600px', margin: '0 auto 48px', lineHeight: 1.8 }}>
      ...
      <section style={{ padding: '80px 24px', background: 'rgba(15,22,41,0.5)' }}>
        <div className="container">
          <h2 style={{ textAlign: 'center', fontSize: '2rem', marginBottom: '48px' }}>이런 기능이 있어요</h2>
          <div className="grid-3">
            {[
              { icon: '🎯', title: '레벨 테스트', desc: 'A1부터 C2까지 자동으로 내 한국어 수준을 파악해요.' },
              { icon: '📚', title: 'i+1 원리 텍스트', desc: '90% 이해 + 10% 새 어휘로 자연스러운 언어 습득을 돕습니다.' },
              { icon: '👆', title: '단어 클릭 사전', desc: '모르는 단어를 클릭하면 정의, 예문, 번역이 즉시 표시됩니다.' },
              { icon: '📝', title: '내 단어장', desc: '저장한 단어를 주제별로 정리해서 언제든 복습할 수 있어요.' },
              { icon: '✅', title: '읽기 진도 저장', desc: '이미 읽은 글은 표시되어 학습 기록을 관리할 수 있어요.' },
              { icon: '🌏', title: '영어 & 스페인어', desc: '영어 또는 스페인어로 맞춤 번역을 제공합니다.' },
            ].map(feature => (
      ...
      <footer style={{ padding: '32px 24px', borderTop: '1px solid var(--border-subtle)', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.875rem' }}>
        <div className="container">📚 읽기 — 한국어 읽기 학습 플랫폼</div>
      </footer>
    </div>
  );
}
