...
async function fetchArticleBatch(options: ListQuery, pageSize = 24, full = false) {
  ...
  const query = {
    ...
    ...(full ? {} : { select: { fields: ['title', 'summary', 'summaries', 'summaryLanguage', 'level', 'topicCategory', 'estimatedMinutes', 'keyVocabulary', 'createdAt', 'averageRating', 'ratingCount', 'imageUrls'].map(fieldPath => ({ fieldPath })) } }),
...
export async function publicArticleIndex(full = false) {
  ...
  do {
    const result = await fetchArticleBatch({ level: 'all', topic: 'all', sort: 'newest', cursor }, 500, full);
...
export async function publicArticlePage(options: ListQuery) {
  ...
  const cursor = z.object({ offset: z.number().int().min(0).max(45000), fingerprint: z.literal(fingerprint) }).parse(raw);
  const items = (await catalog()).filter(a => (options.level === 'all' || a.level === options.level) && (options.topic === 'all' || a.topicCategory === options.topic));
  items.sort((a,b) => (options.sort === 'rating' ? (b.averageRating || 0) - (a.averageRating || 0) : 0) || (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0) || b.id.localeCompare(a.id));
  const end = cursor.offset + 24;
  return { articles: items.slice(cursor.offset, end), cursor: end < items.length ? Buffer.from(JSON.stringify({ offset: end, fingerprint })).toString('base64url') : null };
}
