...
export default function ArticleReader({ initialArticle }: { initialArticle: Article }) {
  ...
  return (
    ...
    >
      ...
      <TutorPanel selected={tutorSelection} onClose={() => setTutorSelection(null)} language={profile?.nativeLanguage || guestLanguage} level={profile?.level || article?.level || 'A1'} labels={t} />
    </div>
  );
}
